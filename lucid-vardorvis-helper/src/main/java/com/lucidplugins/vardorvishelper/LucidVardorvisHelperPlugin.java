package com.lucidplugins.vardorvishelper;

import com.google.inject.Provides;
import com.lucidplugins.vardorvishelper.api.util.CombatUtils;
import com.lucidplugins.vardorvishelper.api.util.InteractionUtils;
import com.lucidplugins.vardorvishelper.api.util.NpcUtils;
...
public class LucidVardorvisHelperPlugin extends Plugin
{
    ...
    private void handleReAttack()
    {
        ...
        final NPC vard = NpcUtils.getNearest("Vardorvis");
        if (vard == null)
        {
            return;
        }
    ...
    {
        Widget[] bloodCaptchaWidgets = client.getWidgets()[BLOOD_CAPTCHA_GROUP_ID];

        List<Widget> captchaWidgets = new ArrayList<>();

        if (bloodCaptchaWidgets == null)
        {
            return captchaWidgets;
        }

        for (Widget w : bloodCaptchaWidgets)
        {
            if (hasAction(w, "Destroy") && !w.isSelfHidden())
            {
